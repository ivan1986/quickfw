...
1) сделать функцию хедер для выноса css и js вверх, в основной шаблон модуля
	...
	При кешировании нужно вместе с блоком кешировать результаты всех вызовов head из него
	Сейчас если значение блока берется из кеша - head остается пустым

	Нуно курить с кешем и смарти

2) мысля по разбиению на папки - заменять в урле символ типа - или _ на / тем самым делая разбитие контроллеров по директориям
сделано

3) для функции добавить переменную кеш, для сайта сделать настройку дефолтное состояние переменной тем самым можно указывать, кешировать состояние или нет для каждого блока, есть проблема - как проверять установлена переменная или нет до запуска, так как лезть каждый раз для некешируемой страницы в кеш - фигня.

4) мысля по поводу сохранения кеширования - массив закешированных шаблонов (их id) класть в кеш, при старте извлекать, и если что не запускать

5) вообще по хорошему нужно разделить шаблоны по модулям, модуль нельзя просто кинуть в папку app и чтобы он заработал, а совпадение шаблонов двух разных модулей красиво решается с помошью жестких или символически ссылок, помоему это лучше, чем лишать возможности и забивать шаблоны вместе
сделано

6) мысль по поводу вызова файлов без роутера - если не хочет человек юзать контроллеры, то можно сделать такое - если файл найден, то вызываем инициализацию (подрубление базы, настройка основных фич, кстати а может сделать два вида вызова существующих файлов - в виде plainPHP или если это сразу шаблон смарти), а затем меняем директорию и вызываем файл.
Или это я изврасранец?

7) нужно сделать процедуру определения - где я и какая страница сейчас генерится - возврат MCA
сделано
в роутере есть три переменных - текущий uri, текущий блок, родительский блок

8) Переписать интерфейс подключения блока - сейчас завязан на интерфейс смарти
сделано
...
index file - по умолчанию false, если у нас на хостинге запрещен .htaccess, чтобы работало хоть как-то

def ext - для SEOнистов (кто это такие - к Валере ;) (нифига. К Диме Смирнову ;-) (вобщем - что это такое - к Диме Смирнову, кто это такой - к Валере :-P)
все урл в генерируемом документе выглядят к base_url/index.php/$1.defext
-- а нафига? некрасиво же
Согласен, но есть заказчики, которым нужно обязательно .html в конце, иначе не кошерно
...
module/controller/action/p1/p2/p3/p4 - значения
...
block:<module>/<controller>/<action>/p1/p2/p3/p4
block:<module>.<controller>.<action>(p1,p2,p3,p4)

сделано, так как способ удобный, то он по умолчанию


при вызове блоков и экшенов по совпадающим урлам не происходит вызова одной и той-же функции
при вызове из URL module/controller/show вызывается ShowAction
при вызове из блока - ShowBlock
...
Появилась документация по основным либам, но ее нужно приводить к нормальному виду - вырезать оформление.
Добавил библиотеку jQuery, у которой вырезана часть, ответственная за AJAX (так как есть JsHttp), если вырезал что-то лишнего - могут быть глюки :)
Библиотека пожата - убраны комменты и пробелы, переменные заменены на односимвольные, eval не использовался
http://dean.edwards.name/packer/
Оригинальную если нужно - берите на сайте и заменяйте.
...
Вопрос в том, как сделать вызов из PlainView блока, наверное нужно еще один способ передачи...
Прикрутил проксирующий шаблонизатор.

----------------------------------------------------------------------------------
Для связи с разработчиками, консультаций, поддержки и всего типа этого
(может сайт закажете сделать :) например или проконсультирую чего по части движка)
E-mail: ivan1986@list.ru
Jabber: ivan1986@jabber.ru
ICQ (для тех изврасранецев, которые до сих пор jabber не завели): 6002482
----------------------------------------------------------------------------------

ОПИСАНИЕ ЧИТАТЬ ОТСЮДА: (вверху были наши мысли, переживания, чувства, детские комплексы, привычки нехорошие и все такое)
                                                    - А почему это недокументировано??
                                                    - Как это недокументировано?! Почитай исходники!
...
Также если вы вдруг решили поменять шаблонизатор, то есть специальный "проксирующий" шаблонизатор, который в зависимости от расширения файла вызывает определенный шаблонизатор.
...
Кеширование - прикрученна библиотека DkLab_Cache с поддержками тегов, немного подпилина, осталось убрать инициализацию интерфейса.
...
QFW::$libs['name'] = new ClassName();

В папке ./application/ размещается ваше приложение.
...
Для блоков будет вызвана функция, оканчивающаяся на 'Block', также вызвав $router->route($url,$type); можно указать тип функции, которая будет вызвана, например для вызова действия из консоли по крону или какой-то программой по событию

далее следуют параметры - есть два варианта передачи параметров - по имени и по позиции
при передачи по позиции вызов
<module>/<controller>/<action>/p1/p2/p3/p4
преобразуется в вызов функции
<action>Action($p1,$p2,$p3,$p4) из контроллера <controller>

при передаче параметров по имени первый параметр пустой - в адресе идут подрят два слеша
вызов
<module>/<controller>/<action>//p1/z1/p2/z2/...
преобразуется в вызов функции
<action>Action() из контроллера <controller>
...
в частности по русски могут называться модули, контроллеры и экшены/блоки

Идея контроллеров была взята из ZendFramework, но от MVC остался только контроллер :)

Для смарти был написан ресурс block - это компонент - как-бы маленькая страничка встроенная в другую.
для его пдключения в смарти пишется что-то похожее на
{include file="block:<module>/<controller>/<action>/p1/p2/p3"}
при этом управление передается функции actionBlock из контроллера и модуля - правила формирования как в url
