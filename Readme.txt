...
В случае, если конструктор вызывает только $this->session() стартуют простые сесии
...
Первый параметр - имя авторизации - соответствует элементу массива $_SESIION ($this->userdata ссылка на $_SESSION[$name])
...
$router, $view и $db, а также $globalData и	$libs доступны как статические члены класса QFW
