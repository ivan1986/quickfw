...
остальные параметры добавляются в массивы _GET и _REQUEST в виде /параметр/значение/ и в случае нечетного количества оставщихся - значение последнего - пустая строка
...
в файле application/config/config.ini указаны основные параметры модулей

; Site configuration data
[host]
host     = photogal.my - хост (пока не юзается, хз зачем)
encoding = utf-8       - кодировка страниц, выставляется при инициализации
...
[state]                - состояния, пока в таком виде
release     = true;    - сейчас от него зависит использование кешера
autosession = true;    - автоматическое создание сессий
def_tpl     = main;    - шаблон, который выводится в конце, чтобы не выводился никакой - оставьте поле пустым

[cacher]               - параметры кеширования
module = File          - какой кешер используется
