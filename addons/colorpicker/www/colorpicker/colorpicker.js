function colorPickerInit(colors) {

	for(var i=0; i < colors.length; i++) {
		var elem = document.getElementById('SC_'+colors[i]+'_input')
		if(!elem) { continue }

		elem.colorSquare = document.getElementById('SC_'+colors[i])
		if(elem.colorSquare) {
			elem.colorSquare.input = elem
			elem.colorSquare.style.cursor = 'pointer'
			elem.colorSquare.onclick = function() { this.input.focus() }
		}

		elem.checkColor = function() {
			if(/^#[0-9abcdefABCDEF]{6}$/.test(this.value.replace(/(^\s+)|(\s+$)/g, ""))) {
				this.className = '';
				return true
			}
			else {
				this.className = 'bad-color';
				return false
			}
		}

		elem.applyColor = function() {
			if(this.colorSquare) {
				var c = parseColor(this.value)
				this.colorSquare.style.backgroundColor = '#'+hex(c.r)+hex(c.g)+hex(c.b)
			}
		}

		elem.onkeydown = elem.onkeyup = elem.onchange = elem.onblur = function() {
			if(this.checkColor()) {
				this.applyColor()
			}
		}

		window.attachColorPicker(elem, false)
	}
...
(function colorPickerNamespace(){
	...
	function createColorPicker(noLg){
		if(noLg) {
			var miniIcon = 'cp_close_icon.gif';
			var onmd = hideColorPicker
		}
		else {
			var miniIcon = 'cp_mini_icon.png';
			var onmd = showLgColorPicker
		}
			...
			['DIV', {style:'position:absolute;font-family:tahoma,verdana,sans-serif;font-size:10px;', ref:'ColorPicker'},
				['DIV', {style:'background-color:#FFF;line-height:2px;width:423px;_width:424px;height:21px;border:1px solid #000;', ref:'hColorPicker'},
					...
					['DIV', {style:'float:left;'},
						...
						['IMG', {style:'cursor:pointer;margin-left:1px;', src:imgBase+miniIcon, width:21, height:21, ref:'hColorIcon', onmousedown:onmd}]
					]
				],
	...
	function hColorPickerMouseDown(){
		...
		if(typeof cp.cpColor == 'object') cp.cpColor = '#'+hex(cp.cpColor.r)+hex(cp.cpColor.g)+hex(cp.cpColor.b);

		cp.cpInput.value = cp.cpColor;
		cp.cpInput.applyColor();

		hideColorPicker();
	}

	function attachColorPicker(input, noLg){
		createColorPicker(noLg);
	...
	function showColorPicker(ev, size){
		...
		cp.hColorIcon.style.display = 'inline'

		var inpPos = getPosition(cp.cpInput);

		cp.ColorPicker.style.left = (inpPos.x+parseInt(cp.cpInput.offsetWidth))+'px';
		cp.ColorPicker.style.top  = inpPos.y+'px';
