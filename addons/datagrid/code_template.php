...
## Файл подключается в функцию - один для таблицы, после чего настраивается,
## и по выходу в переменной $table находится сгенерированная таблица
## Вобщем потом допиливать
################################################################################
## +---------------------------------------------------------------------------+
## | 1. Creating & Calling:                                                    |
## +---------------------------------------------------------------------------+
...
##  *** creating variables that we need for database connection
  global $config;
  $DB_USER=$config['database']['username'];   /* usually like this: prefix_name             */
  $DB_PASS=$config['database']['password'];   /* must be already enscrypted (recommended)   */
  $DB_HOST=$config['database']['host'];       /* usually localhost                          */
  $DB_NAME=$config['database']['dbname'];     /* usually like this: prefix_dbName           */
...
##  До этого комментария менять возможно придется только $debug_mode и $messaging
##  Далее идет конфигурация таблицы, для простейшей таблицы раскомментировано
##  Что закомментировано /// - необязательные опции

##  *** Запрос, для таблицы - первое поле считается PrimaryKey
...
##
##  *** Установка кодировки (default: utf8/utf8_unicode_ci)
  $dg_collation = "utf8_unicode_ci";
  $dgrid->setEncoding($config['database']['encoding'], $dg_collation);
##
## +---------------------------------------------------------------------------+
## | 2. General Settings:                                                      |
## +---------------------------------------------------------------------------+
