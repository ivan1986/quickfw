...
class Jabber
{
	...
	function Jabber()
	{
		...
		$this->iq_version_os		= 1;//$_SERVER['SERVER_SOFTWARE'];
	...
	function Connect()
	{
		...
		if ($this->CONNECTOR->OpenSocket($this->host, $this->port))
		{
	...
	function SendAuth()
	{
		...
		$this->resource	= ($this->resource != NULL) ? $this->resource : ("Bot");//Class.Jabber.PHP " . md5($this->auth_id));
	...
	{
		if ($to && is_array($content) && isset($content['body']))
		{
			if (!$id)
			{
				$id = $type . "_" . time();
				...
				&& $incoming_array['iq']['#']['query']['0']['@']['xmlns'] == "jabber:iq:roster")
			{
				...
				for ($a = 0; $a < $number_of_contacts; $a++)
				{
					$this->roster[$a] = array(
						...
						"group"			=> isset($incoming_array['iq']['#']['query'][0]['#']['item'][$a]['#']['group'])?
							$incoming_array['iq']['#']['query'][0]['#']['item'][$a]['#']['group'][0]['#']:0
					);
				}
		...
		while (++$i < count($vals))
		{
			switch ($vals[$i]['type'])
			{
				...
				case 'complete':
					...
					$children[$tagname][$size]['#'] = trim(isset($vals[$i]['value']) ? $vals[$i]['value'] : '');
