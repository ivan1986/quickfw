...
class HTML_MetaForm extends HTML_SemiParser
{
	 ...
	 * as POST action).
	 */
	public function HTML_MetaForm($signature)
	{
		$this->HTML_SemiParser();
		$this->_mf_hash = dechex(crc32(microtime(true) . uniqid("") . getmypid()));
		...
		$this->MF_REQUEST_URI = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '';
		$this->MF_POST = isset($_SERVER['REQUEST_METHOD']) && 
			strtoupper($_SERVER['REQUEST_METHOD']) == 'POST' ? $this->_getFullPostData() : null;
	}
	...
	function tag_form($tag)
	{
		...
		$tag['_right'] = $hash . (isset($tag['_right']) ? $tag['_right'] : '');
	...
	{
		if (!isset($tag['name']) || !isset($tag['type']) || strtolower($tag['type']) !== 'submit')
			return;
	...
	function _addItems($type, &$tag, $items)
	{
		...
		$hash = $this->_getHash();
		$tag['_right'] = (isset($tag['_right']) ? $tag['_right'] : '') . $hash;
...
//            printr($metas);
			...
			if (isset($formTag['method']) && strtoupper($formTag['method']) == 'POST')
			{
				// Generate hidden tag.
	...
	function _getMetasInText(&$content)
	{
		...
		{
			if (!isset($this->_mf_collectMetas[$hash]))
				continue;
			if (!is_array($this->_mf_collectMetas[$hash]))
				continue;
		...
		foreach ($eltMetas as $eltMeta)
		{
			...
			if (!isset($eltMeta['label']) &&
				isset($eltMeta['id']) &&
				isset($this->_mf_collectLabels[$eltMeta['id']]))
			{
			...
			if ($meta['type'] == "multiple")
			{
				...
				if (substr($name, -2) == '[]')
				{
					...
					$meta['items'][$eltMeta['key']] = isset($eltMeta['label']) ? $eltMeta['label'] : '';
		...
		foreach ($flatMetas as $k => $meta)
		{
			...
			if ($this->MF_USE_VALUES)
			{
				...
				if ($meta['type'] == "action" && count($nameParts) == 1)
				{
					...
					if (isset($valuesArray[$name.'_x'])
						&& is_numeric($x = $valuesArray[$name.'_x'])
						&& isset($valuesArray[$name.'_y'])
						&& is_numeric($y = $valuesArray[$name.'_y']))
					{
						$value = array($x, $y);
					}
				} 
